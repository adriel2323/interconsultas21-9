...
<div class="row">
    <div class="col-md-12">
        ...
        <div style="text-align: center;">
            <h3>Listado de muestras recibidas y no informadas.</h3>
        </div>
    </div>
</div>
<div class="row">
        ...
        <table class="table table-bordered">
            ...
            @foreach($samples as $sample)
                <tr>
                    ...
                    <td>{{ \Carbon\Carbon::parse($sample->received_at)->format('d/m/Y H:i') }}</td>
                    <td>{{ $sample->department->name }}</td>
                    <td>{{ $sample->code }}</td>
                    <td>{{ isset($sample->surgeryEvent) ? $sample->surgeryEvent->EventData->patient_name : $sample->patient->apellido }}</td>
                </tr>
            @endforeach
            </tbody>
        </table>
