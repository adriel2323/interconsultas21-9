...
<div class="row">
    <div class="col-md-12">
        ...
        <div style="text-align: center;">
            <h3>Listado de cirugías realizadas entre el {{\Carbon\Carbon::parse($from)->format('d/m/Y')}} y el {{\Carbon\Carbon::parse($to)->format('d/m/Y')}} realizadas por {{ $surgeon->name }}</h3>
        </div>
    </div>
</div>
<div class="row">
    ...
    <div class="col-sm-12">
        ...
        <div class="pull-left">
            <ul>
                ...
                @foreach($os as $row)
                    <li><b>Cantidad de cirugías de {{ $row['name'] }}: {{ $row['total'] }}</b></li>
                @endforeach
            </ul>
        </div>
    </div>
